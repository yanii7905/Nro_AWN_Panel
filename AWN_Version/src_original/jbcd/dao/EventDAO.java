...
public class EventDAO {
    ...
    public static void save() {
        try {
            ...
            int rows1 = ConnectDB.executeUpdate("UPDATE `event` SET `data` = ? WHERE `name` = 'international_womens_day'", jsonWomenDay.toString());
            if (rows1 == 0) Logger.warning("Không cập nhật được event 'international_womens_day'");
            ...
            int rows2 = ConnectDB.executeUpdate("UPDATE `event` SET `data` = ? WHERE `name` = 'luna_new_year'", jsonTet.toString());
            if (rows2 == 0) Logger.warning("Không cập nhật được event 'luna_new_year'");
            ...
            int rows3 = ConnectDB.executeUpdate("UPDATE `event` SET `data` = ? WHERE `name` = 'christ_mas'", jsonXmas.toString());
            if (rows3 == 0) Logger.warning("Không cập nhật được event 'christ_mas'");
            
            // Hùng Vương
            JsonObject jsonhungkings = new JsonObject();
            jsonhungkings.addProperty("ReceiveMelonSeed", RECEIVE_MELON_SEED);
            int rows4 = ConnectDB.executeUpdate("UPDATE `event` SET `data` = ? WHERE `name` = 'hung_vuong'", jsonhungkings.toString());
            if (rows4 == 0) Logger.warning("Không cập nhật được event 'hung_vuong'");
