...
public class Doi extends Boss {
    ...
    public Doi() throws Exception {
        super(HALLOWEEN_EVENT, BossID.DOI, false, true, false, false, BossesData.DOI);
    }

    @Override
    public void reward(Player plKill) {
        if (InventoryService.gI().findGioDungNgocBi(plKill)) {
            ItemMap it = new ItemMap(this.zone, 704, 1, this.location.x, this.zone.map.yPhysicInTop(this.location.x, this.location.y - 24), plKill.id);
    ...
    public void halloween(Player player) {
        if (player.effectSkill != null && !player.effectSkill.isHalloween) {
            EffectSkillService.gI().setIsHalloween(player, 0, 1800000);
        }
    }
    ...
    @Override
    public void joinMap() {
        this.name = "Dơi " + Util.nextInt(1, 150);
        super.joinMap();
