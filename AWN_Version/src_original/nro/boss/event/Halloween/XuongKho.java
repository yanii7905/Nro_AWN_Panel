...
public class XuongKho extends Boss {

    public XuongKho() throws Exception {
        super(HALLOWEEN_EVENT, BossID.XUONG_KHO, false, true, false, false, BossesData.XUONG_KHO);
    }

    @Override
    public void reward(Player plKill) {
        if (InventoryService.gI().findGioDungNgocBi(plKill)) {
            ItemMap it = new ItemMap(this.zone, 702, 1, this.location.x, this.zone.map.yPhysicInTop(this.location.x, this.location.y - 24), plKill.id);
        ...
        for (int i = 0; i < Util.nextInt(1, 3); i++) {
            ItemMap it = new ItemMap(this.zone, 585, 1, this.location.x + Util.nextInt(-50, 50), this.zone.map.yPhysicInTop(this.location.x, this.location.y - 24), plKill.id);
            Service.gI().dropItemMap(this.zone, it);
        }
    }
    ...
    @Override
    public void joinMap() {
        this.name = "Xương khô " + Util.nextInt(1, 150);
