    ...
    public void reward(Player plKill) {
        ...
        if (EventManager.CHRISTMAS) {
            for (int i = 0; i < Util.nextInt(10, 50); i++) {
                ItemMap it = new ItemMap(this.zone, 1462, 1, this.location.x + Util.nextInt(-100, 100), this.zone.map.yPhysicInTop(this.location.x, this.location.y - 24), -1);
                it.addOptionParam(86, 0);
                it.addOptionParam(93, 30);
                Service.gI().dropItemMap(this.zone, it);
            }
            for (int i = 0; i < Util.nextInt(3, 10); i++) {
                ItemMap it = new ItemMap(this.zone, 925, 1, this.location.x + Util.nextInt(-100, 100), this.zone.map.yPhysicInTop(this.location.x, this.location.y - 24), -1);
            ...
            for (int i = 0; i < Util.nextInt(5, 20); i++) {
                ItemMap it = new ItemMap(this.zone, Util.nextInt(1446, 1447), 1, this.location.x + Util.nextInt(-100, 100), this.zone.map.yPhysicInTop(this.location.x, this.location.y - 24), -1);
                it.addOptionParam(30, 0);
                it.addOptionParam(93, 30);
                Service.gI().dropItemMap(this.zone, it);
            }
            if (Util.isTrue(100, 100)) {
                ItemMap it = new ItemMap(this.zone, 1436, 1, this.location.x, this.zone.map.yPhysicInTop(this.location.x, this.location.y - 24), plKill.id);
