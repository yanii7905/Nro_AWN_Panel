...
public class AnTrom extends Boss {
    ...
    public double injured(Player plAtt, double damage, boolean piercing, boolean isMobAttack) {
        if (!this.isDie()) {
            ...
            if (Util.isTrue(20, 100)) {
                ItemMap it = new ItemMap(this.zone, 189, Util.nextInt(20_000, 25_000), this.location.x + Util.nextInt(-20, 20), this.zone.map.yPhysicInTop(this.location.x,
            ...
            try {
                ...
                if (Util.getDistance(this, pl) <= 40) {
                    ...
                    if (pl.isPl()) {
                        ...
                        this.chat("đã trộm được " + Util.formatNumber(gold, FormatStyle.VIETNAMESE) + " Vàng roài");
                        if (gold > 0) {
    ...
    public void reward(Player plKill) {
        if (goldAnTrom != 0) {
            ...
            for (byte i = 0; i < 5; i++) {
                ItemMap it = new ItemMap(this.zone, 190, (int) (goldAnTrom / 5), this.location.x + Util.nextInt(-50, 50), this.zone.map.yPhysicInTop(this.location.x,
        ...
        if (EventManager.LUNNAR_NEW_YEAR) {
            ...
            for (int i = 0; i < Util.nextInt(1, 5); i++) {
                ItemMap it = new ItemMap(this.zone, 1472, 1, this.location.x + Util.nextInt(-15, 15), this.zone.map.yPhysicInTop(this.location.x, this.location.y - 24), plKill.id);
                it.addOptionParam(86, 0);
                it.addOptionParam(93, 30);
                Service.gI().dropItemMap(this.zone, it);
            }
