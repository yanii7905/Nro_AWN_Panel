...
public class Broly extends Boss {
    ...
    public Broly() throws Exception {        
        super(BROLY, BossID.BROLY, new BossData(
                ...
                new int[][]{
                    ...
                    {Skill.ANTOMIC, Util.nextInt(1, 3), 1500},}, //skill
    ...
    public double injured(Player plAtt, double damage, boolean piercing, boolean isMobAttack) {
        if (!this.isDie()) {
            ...
            if (!piercing) {
                if ((plAtt.playerSkill.skillSelect.template.id == Skill.ANTOMIC || plAtt.playerSkill.skillSelect.template.id == Skill.KAMEJOKO || plAtt.playerSkill.skillSelect.template.id == Skill.MASENKO || plAtt.playerSkill.skillSelect.template.id == Skill.LIEN_HOAN)) {
                    this.chat("Xí hụt");
                    damage = 0;
                }
    ...
    private void angry() {
        if (this.playersAttack.size() < 5 && Util.isTrue(7, ConstRatio.PER100)) {
            ...
            while (i.hasNext()) {
                ...
                if (pl.isPl() && !pl.equals(this) && Util.getDistance(this, pl) <= DIS_ANGRY && !pl.isBoss && !pl.isDie() && !isInListPlayersAttack(pl)) {
                    try {
    ...
    public void attack() {
        try {
            if (!charge()) {
                ...
                this.playerSkill.skillSelect = this.playerSkill.skills.get(Util.nextInt(Util.nextInt(1, 4), this.playerSkill.skills.size() - 1));
