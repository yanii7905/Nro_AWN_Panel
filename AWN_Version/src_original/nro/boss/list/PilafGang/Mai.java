...
public class Mai extends Boss {
    ...
    public void doneChatE() {
        ...
        if (parentBoss == null || parentBoss.bossAppearTogether == null
                || parentBoss.bossAppearTogether[parentBoss.currentLevel] == null) return;
    ...
    public void autoLeaveMap() {
        if (Util.canDoWithTime(st, 900000)) this.leaveMapNew();
        if (zone != null && zone.getNumOfPlayers() > 0) st = System.currentTimeMillis();
    }
    ...
    public synchronized double injured(Player plAtt, double damage, boolean piercing, boolean isMobAttack) {
        if (!this.isDie()) {
            ...
            if (!piercing && effectSkill.isShielding) {
                if (damage > nPoint.hpMax) EffectSkillService.gI().breakShield(this);
                damage = 1;
            }
