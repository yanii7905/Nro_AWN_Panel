...
//Zalo: 0974764064
    ...
    private void kickOutOfMap22h(Player player) {
        if (MapService.gI().isMapHirudegarn(player.zone.map.mapId)) {
            Service.getInstance().sendThongBao(player, "Trận Chiến Đã Kết Thúc, Tàu Vận Chuyển Sẽ Đưa Bạn Về Nhà");
    ...
    public void update(Player player) {
        if (player.zone == null || !MapService.gI().isMapHirudegarn(player.zone.map.mapId)) {
            try {
