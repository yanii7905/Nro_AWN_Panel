...
public class NangCapChanMenh {
    ...
    public static void showInfoCombine(Player player) {
        ...
        if (ChanMenh != null && TinhThe != null && MaQuai != null && (ChanMenh.template.id >= 2002 && ChanMenh.template.id < 2010)) {
            ...
            if (TinhTheQuantity >= CombineUtil.getMaQuaiVaTinhTheNangcapChanmenh(level) && MaQuaiQuantity >= CombineUtil.getMaQuaiVaTinhTheNangcapChanmenh(level)) {
    ...
    public static void nangCapChanMenh(Player player, int... numm) {
        ...
        if (success) {
            if (capbac > 4) {
                ServerNotify.gI().notify("Chúc mừng " + player.name + " vừa nâng cấp thành công " + ChanMenh.template.name + " lên 1 cấp");
            }
            if (n > 1) {
...
//                    io.param += (1 + capbac * Util.nextInt(1, 2)); 
//                }
