...
public class MeoDen extends Boss {
    ...
    public MeoDen() throws Exception {
        super(TET_EVENT, BossID.MEO_DEN - Util.nextInt(10_000, 20_000), BossesData.MEO_DEN);
    }

    @Override
    public void joinMap() {
        this.name = "Mèo đen " + Util.nextInt(1, 100);
        if (zoneFinal != null) {
        ...
        if (this.zone != null) {
            try {
                ...
                timeLeave = Util.nextInt(300_000, 600_000);
            } catch (Exception e) {
    ...
    public void afk() {
        if (Util.canDoWithTime(this.lastTimeAttack, 500)) {
            ...
            if (pl.haveReward_MeoDen) {
                pl.haveReward_MeoDen = false;
                this.leaveMap();
                return;
            }
            if (this.zone.equals(pl.zone)) {
                int dis = Util.getDistance(this, pl);
                if (!pl.itemTime.isUseFoodMeoDen1 && !pl.itemTime.isUseFoodMeoDen2) {
                    afk = true;
                    pl.canReward_MeoDen = false;
                } else if (dis <= 300 && pl.itemTime != null && (pl.itemTime.isUseFoodMeoDen1 || pl.itemTime.isUseFoodMeoDen2)) {
                    if (dis > 50) {
                        int dir = (this.location.x - pl.location.x < 0 ? 1 : -1);
                        int move = Util.nextInt(50, 100);
                        move(this.location.x + (dir == 1 ? move : -move), pl.location.y);
                    ...
                    afk = false;
                    pl.canReward_MeoDen = true;
                } else {
                    afk = true;
                    pl.canReward_MeoDen = false;
                }
            } else if (!afk) {
                if (pl.changeMapVIP_MeoDen) {
                    pl.changeMapVIP_MeoDen = false;
                    pl.canReward_MeoDen = false;
                    afk = true;
                    return;
                }
    ...
    public synchronized double injured(Player plAtt, double damage, boolean piercing, boolean isMobAttack) {
        if (!this.isDie()) {
            ...
            if (damage >= this.nPoint.hp) {
                plAtt.typePk = ConstPlayer.PK_ALL;
                this.playerId = Math.abs(plAtt.id);
                Service.gI().chat(plAtt, "Đi theo ta nào, Mèo đen!");
