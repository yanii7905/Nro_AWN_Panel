    ...
    public boolean addItemList(List<Item> items, Item itemAdd) {
        ...
        if (itemAdd.template.isUpToUp) {
            for (Item it : items) {
                ...
                if (itemAdd.template.id == 1705 || itemAdd.template.id == 457) {// Không Giới Hạn số lượng
                    it.quantity += itemAdd.quantity;
                    itemAdd.quantity = 0;
                    return true;
                }
                if (it.quantity < 9999) {// Giới hạn 9999
                ...
                if (it != null && it.isNotNullItem() && it.template.id == 718) {
                    ...
                    for (ItemOption io : it.itemOptions) {
                        if (io.optionTemplate.id == 31) {
                            ...
                            if (total > 30_000) {
                                io.param = 30_000;
                                Service.getInstance().sendThongBao(player, "Số lượng đã đạt tối đa");
                            } else {
                                io.param = total;
                            }
                            found = true;
                    ...
                    if (!found) {
                        // Nếu không có option 31, thêm mới
                        int value = Math.min(item.quantity, 30_000);
                        it.itemOptions.add(new ItemOption(31, value));
                        if (item.quantity > 30_000) {
                            Service.getInstance().sendThongBao(player, "Số lượng đã đạt tối đa");
                        }
                    }
                    item.quantity = 0;
