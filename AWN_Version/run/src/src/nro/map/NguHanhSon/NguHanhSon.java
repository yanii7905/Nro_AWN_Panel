...
public class NguHanhSon {
    ...
    private void kickOutOfnguhs(Player player) {
        if (MapService.gI().isMapNguHanhSon(player.zone.map.mapId)) {
            Service.getInstance().sendThongBao(player, "|7|Ngũ Hành Sơn Đã Đóng, Bạn Sẽ Bị Kik Ra Khỏi Map!");
    ...
    public void update(Player player) {
        try {
            ...
            if (!(now > TIME_OPEN_NHS && now < TIME_CLOSE_NHS) && MapService.gI().isMapNguHanhSon(player.zone.map.mapId)) {
