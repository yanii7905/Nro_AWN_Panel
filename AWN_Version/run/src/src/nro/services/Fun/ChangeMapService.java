...
public class ChangeMapService {
    ...
    public void changeZone(Player pl, int zoneId) {
        ...
        if (pl.isFounder() || pl.isQuanTriVien() || pl.isBoss || Util.canDoWithTime(pl.iDMark.getLastTimeChangeZone(), 20000)) {
            if (pl.itemTime.isUseTDLT) {
                pl.iDMark.setLastTimeChangeZone(System.currentTimeMillis() - 15000);
            } else {
            ...
            Service.gI().sendThongBaoFromAdmin(pl, "Đổi khu quá nhanh, vui lòng đợi " + TimeUtil.getTimeLeft(pl.iDMark.getLastTimeChangeZone(), 20));
    ...
    public void changeMapWaypoint(Player player) {
        ...
        if (zoneJoin != null) {
            ...
            if (player.zone.map.mapId != 21 + player.gender) {
                if (!Util.canDoWithTime(player.MapTransitionTime, 1000)) {
                    resetPoint(player);
