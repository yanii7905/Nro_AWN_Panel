            ...
            if (pl != null) {
                if (player.isFounder() || player.nPoint.teleport) {
                    if (!pl.itemTime.isUseAnDanh || player.isFounder()) {
                        if ((player.isFounder() || !pl.zone.isFullPlayer()) && !MapService.gI().isMapOffline(pl.zone.map.mapId) && !MapService.gI().isMapBangHoi(pl.zone.map.mapId) && !MapService.gI().isMapBlackBallWar(pl.zone.map.mapId) && !MapService.gI().isMapPhoBan(pl.zone.map.mapId) && !MapService.gI().isMapMaBu12H(pl.zone.map.mapId)) {
