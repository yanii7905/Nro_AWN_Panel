    ...
    public Player getPlayerCanAttack() {
        ...
        try {
            ...
            for (Player pl : players) {
                if (!pl.isDie() && !pl.isBoss && !pl.isPetFollow && !pl.isDuongTang && (pl.satellite == null || !pl.satellite.isDefend) && (pl.effectSkin == null || !pl.effectSkin.isVoHinh) && (this.tempId > 18 || (this.tempId > 9 && this.type == 4)) || isBigBoss()) {
...
//            msg.writeLong(this.point.hp);
    ...
    public List<ItemMap> getItemMobReward(Player player, int x, int yEnd) {
        ...
        // -------------------DROP CAPSULE----------------------------
        if (player.itemTime != null && player.itemTime.isUseMayDo && MapService.gI().isMapTuongLai(mapid) && (Util.isTrue(1, 10))) {
...
// ------------------ DROP THỨC ĂN KHI MANG ĐỒ THẦN LINH ------------------
        if (!MapService.gI().isMapTuongLai(mapid)
            ...
            if (hasThanLinh) {
                ...
                int[] thucAnList = MobDrop.list_thuc_an; // {663, 664, 665, 666, 667}
