...
//                }
    ...
    public void showConfirmSellItem(Player pl, int where, int index) {
        ...
        if (item != null && item.isNotNullItem()) {
            ...
            if (item.template.id == 457) {
                if (quantity > 1) {
                    Input.gI().createFormBanSoLuongLonThoiVang(pl);
                    return;
                }
                quantity = 1;
            } else {
            ...
            String text = "Bạn có muốn bán\nx" + quantity
                    + " " + item.template.name + "\nvới giá là " + Util.formatNumber(cost, FormatStyle.VIETNAMESE) + " vàng?";
            Message msg = null;
            try {
                msg = new Message(7);
    ...
    public void sellItem(Player pl, int where, int index) {
        ...
        if (item != null) {
            ...
            Service.gI().sendThongBao(pl, "Đã bán " + item.template.name + " thu được " + Util.formatNumber(cost, FormatStyle.VIETNAMESE) + " vàng");
            if (item.template.id != 457) {
            ...
            if ("BUNMA".equals(pl.iDMark.getTagNameShop()) || "DENDE".equals(pl.iDMark.getTagNameShop()) || "APPULE".equals(pl.iDMark.getTagNameShop())) {
