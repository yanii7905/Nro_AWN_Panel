package nro.boss.dungeon.BossOfTheGangs;
...
public class ThoBunmaClan extends Boss {
    ...
    public ThoBunmaClan(Zone zone, Clan clan) throws Exception {
        super(PHOBANBBH, BossID.THO_BUNMA_CLAN, new BossData(
                ...
                new String[]{},
                new String[]{"|-1|Em ơi đừng khóc bóng tối trước mắt sẽ bắt em đi",
                ...
                "|-1|Sau bao niêm đau em mong rằng con tim em dừng lại",
    ...
    private void dropCt(int x) {
        ItemMap it = new ItemMap(zone, 584, 1, this.location.x + x, this.zone.map.yPhysicInTop(this.location.x, this.location.y - 24), -1);
        it.addOptionParam(50, 24);
        it.addOptionParam(117, 15);
        it.addOptionParam(114, 25);
    ...
    private void dropItem() {
        for (int i = 0; i < Util.nextInt(4, 5); i++) {
            ItemMap it = new ItemMap(zone, 1150, 1, this.location.x + Util.nextInt(-250, 250), this.zone.map.yPhysicInTop(this.location.x, this.location.y - 24), -1);
            it.addOptionParam(86, 0);
            it.addOptionParam(30, 0);
            Service.gI().dropItemMap(this.zone, it);
        }
        for (int i = 0; i < Util.nextInt(4, 5); i++) {
            ItemMap it = new ItemMap(zone, 1151, 1, this.location.x + Util.nextInt(-250, 250), this.zone.map.yPhysicInTop(this.location.x, this.location.y - 24), -1);
            it.addOptionParam(86, 0);
            it.addOptionParam(30, 0);
            Service.gI().dropItemMap(this.zone, it);
        }
        for (int i = 0; i < Util.nextInt(4, 5); i++) {
            ItemMap it = new ItemMap(zone, 1152, 1, this.location.x + Util.nextInt(-250, 250), this.zone.map.yPhysicInTop(this.location.x, this.location.y - 24), -1);
            it.addOptionParam(86, 0);
            it.addOptionParam(30, 0);
            Service.gI().dropItemMap(this.zone, it);
        }
        for (int i = 0; i < Util.nextInt(4, 5); i++) {
            ItemMap it = new ItemMap(zone, 1153, 1, this.location.x + Util.nextInt(-250, 250), this.zone.map.yPhysicInTop(this.location.x, this.location.y - 24), -1);
            it.addOptionParam(86, 0);
            it.addOptionParam(30, 0);
            Service.gI().dropItemMap(this.zone, it);
        }
        for (int i = 0; i < Util.nextInt(4, 5); i++) {
            ItemMap it = new ItemMap(zone, 1154, 1, this.location.x + Util.nextInt(-250, 250), this.zone.map.yPhysicInTop(this.location.x, this.location.y - 24), -1);
            it.addOptionParam(86, 0);
            it.addOptionParam(30, 0);
            Service.gI().dropItemMap(this.zone, it);
        }
