...
//            }
    ...
    private void playerAttackPlayer(Player plAtt, Player plInjure, boolean miss) {
        ...
        double dameAttack = (Manager.readInt == true ? plAtt.nPoint.getDameAttack(false) : Util.CrisGH(plAtt.nPoint.getDameAttack(false)));
        ...
        if (plAtt.isPlMan()) {
            ...
            if (tlDameBoss > 0 && plInjure.isBoss) {
                dameAttack += Util.CrisGH((dameAttack / 100) * tlDameBoss);
            }
        }
        ...
        if (plAtt.isPl() && dameHit >= 150_000_000) {
            ServerNotify.gI().notify(plAtt.name + ": đã đánh 1 chiêu " + skillSelect.template.name + " với sát thương là " + Util.formatNumber(Util.CrisGH(dameHit), FormatStyle.VIETNAMESE));
        }
        ...
        try {
            ...
            if (dameHit > Integer.MAX_VALUE && Manager.readInt) {
                if (dameHit > Integer.MAX_VALUE - 10) {
                    Service.getInstance().sendThongBao(plAtt, "|2|Sát Thương ➻ ️" + " \b|0|" + "-" + Util.formatNumber(dameHit, FormatStyle.VIETNAMESE));
                }
                dameHit = Integer.MAX_VALUE - 9;
                msg.writeCris(Util.CrisGH(dameHit), Manager.readInt); //dame ăn
            } else {
                msg.writeCris(Util.CrisGH(dameHit), Manager.readInt); //dame ăn
            }
            ...
            if (plAtt.isPl() && plInjure.isPl() && plAtt.typePk == ConstPlayer.PK_PVP_2 && plInjure.typePk == ConstPlayer.PK_PVP_2) {
                long tnsm = plAtt.nPoint.calSucManhTiemNang(Util.CrisGH(dameHit / 10)) / (Math.abs(Service.gI().getCurrLevel(plAtt) - Service.gI().getCurrLevel(plInjure)) + 1);
                Service.gI().addSMTN(plInjure, (byte) 2, tnsm, false);
            }
            if (plInjure.isDie() && !plAtt.isBoss && !plInjure.isBoss && MapService.gI().isMapMaBu12H(plInjure.zone.map.mapId)) {
            ...
            if (dameHit < Integer.MAX_VALUE && plInjure instanceof TestDame) {
                ...
                Service.getInstance().sendThongBao(plAtt, "|0|Dame Thật: \b|5|" + Util.format(dameHit) + "\n\n|1| Tổng DAME Trong 5 Giây: \b|7|" + Util.formatNumber(plAtt.dametong, FormatStyle.VIETNAMESE));
